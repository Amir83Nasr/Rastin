/* ═══════════════════════════════════════════════════════════════
   ...
   •  Safe no-throw chrome.storage access (survives reload/update)
   ═══════════════════════════════════════════════════════════════ */
...
(function () {
  ...
  RastinLogger.prototype._write = function (level, code, message, context) {
    ...
    if (this.opts.consoleOutput) {
      ...
      var consoleMsg = message;
      if (context && typeof context === 'object' && Object.keys(context).length > 0) {
        try {
          consoleMsg += ' ' + JSON.stringify(context);
        } catch (_) {}
      }
      if (level >= LEVEL.ERROR) {
        console.error(tag, consoleMsg);
      } else if (level >= LEVEL.WARN) {
        console.warn(tag, consoleMsg);
      } else {
        console.log(tag, consoleMsg);
      }
    }
  ...
  RastinLogger.prototype.flush = function () {
    ...
    try {
      ...
      return; // extension context invalidated — drop batch silently
    }

    try {
      chrome.storage.local.get([this.opts.storageKey], function () {
        try {
          var args = Array.prototype.slice.call(arguments);
          var result = args[0] || {};
          if (chrome.runtime && chrome.runtime.lastError) return; // context invalidated
      ...
      /* best-effort — context invalidated */
    }
  };
  ...
  RastinLogger.prototype.getLogs = function (filter) {
    ...
    return new Promise(function (resolve) {
      ...
      chrome.storage.local.get([self.opts.storageKey], function (result) {
        ...
        var logs = (result && result[self.opts.storageKey]) || [];
        if (filter) {
  ...
  */
  ...
  /* ─── User-facing toast notification ────────────────
     ...
     Types: 'info' (blue), 'warn' (amber), 'error' (red),
            'success' (dark #101010 + cream #f3f4ed).
     Duration: 3s for info/warn, 4s for success, 6s for error. */
  RastinLogger.prototype.notify = function (message, type, duration) {
    ...
    if (duration === undefined)
      duration =
        type === 'error' ? 6000 : type === 'warn' ? 4000 : type === 'success' ? 4000 : 3000;

...
(function () {
  ...
  // ─── Persian Font Toggle (independent of RTL/font persistence) ──
  /**
   * Apply IRANYekanX font to the page.
   * Uses CSS classes defined in content.css so no style injection needed.
   * For scoped sites (e.g. GitHub README), uses a data attribute instead.
   */
  function applyPersianFont() {
    var scoped = document.querySelector('[data-rastin-rtl]');
    if (scoped) {
      scoped.setAttribute('data-rastin-persian-font', '');
      return;
    }
    document.documentElement.classList.add('rtl-translator-persian-font');
  }

  function removePersianFont() {
    var scoped = document.querySelector('[data-rastin-persian-font]');
    if (scoped) scoped.removeAttribute('data-rastin-persian-font');
    document.documentElement.classList.remove('rtl-translator-persian-font');
  }

  function isPersianFontActive() {
    return (
      document.documentElement.classList.contains('rtl-translator-persian-font') ||
      !!document.querySelector('[data-rastin-persian-font]')
    );
  }
  ...
  chrome.runtime.onMessage.addListener(function (message, sender, sendResponse) {
    try {
      switch (message.action) {
        ...
        case 'apply_persian_font':
          applyPersianFont();
          saveState(isRTLActive());
          sendResponse({ success: true, persianFont: true });
          break;

        case 'remove_persian_font':
          removePersianFont();
          saveState(isRTLActive());
  ...
  async function init() {
    ...
    // Restore previous RTL and Persian font state for this domain
    var saved = loadState();
    if (saved) {
      if (saved.active) {
        applyRTL();
